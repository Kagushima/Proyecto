from typing import List
import base64

# -------------------------
# Constantes AES
# -------------------------

s_box = [

]

inv_s_box = [

]

Rcon = [
0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
]

# -------------------------
# Funciones AES
# -------------------------

def sub_bytes(state: List[List[int]]) -> None:
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]
def inv_sub_bytes(state: List[List[int]]) -> None:
    for i in range(4):
        for j in range(4):
            state[i][j] = inv_s_box[state[i][j]]
def shift_rows(state: List[List[int]]) -> None:
    state[1][0], state[1][1], state[1][2], state[1][3] = state[1][1], state[1][2], state[1][3], state[1][0]
    state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
    state[3][0], state[3][1], state[3][2], state[3][3] = state[3][3], state[3][0], state[3][1], state[3][2]
def inv_shift_rows(state: List[List[int]]) -> None:
    state[1][0], state[1][1], state[1][2], state[1][3] = state[1][3], state[1][0], state[1][1], state[1][2]
    state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
    state[3][0], state[3][1], state[3][2], state[3][3] = state[3][1], state[3][2], state[3][3], state[3][0]
def mix_columns(state: List[List[int]]) -> None:
    for j in range(4):
        a = [state[i][j] for i in range(4)]
        state[0][j] = gmul(a[0],2) ^ gmul(a[1],3) ^ a[2] ^ a[3]
        state[1][j] = a[0] ^ gmul(a[1],2) ^ gmul(a[2],3) ^ a[3]
        state[2][j] = a[0] ^ a[1] ^ gmul(a[2],2) ^ gmul(a[3],3)
        state[3][j] = gmul(a[0],3) ^ a[1] ^ a[2] ^ gmul(a[3],2)
def inv_mix_columns(state: List[List[int]]) -> None:
    for j in range(4):
        a = [state[i][j] for i in range(4)]
        state[0][j] = gmul(a[0],14) ^ gmul(a[1],11) ^ gmul(a[2],13) ^ gmul(a[3],9)
        state[1][j] = gmul(a[0],9) ^ gmul(a[1],14) ^ gmul(a[2],11) ^ gmul(a[3],13)
        state[2][j] = gmul(a[0],13) ^ gmul(a[1],9) ^ gmul(a[2],14) ^ gmul(a[3],11)
        state[3][j] = gmul(a[0],11) ^ gmul(a[1],13) ^ gmul(a[2],9) ^ gmul(a[3],14)
def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        if hi_bit_set:
            a ^= 0x1b
        b >>= 1
    return p % 256
def key_generation(key: List[List[int]]) -> List[List[List[int]]]:
    round_keys = [key]
    for round in range(1, 11):
        prev_key = round_keys[-1]
        new_key = [row[:] for row in prev_key]
        # RotWord and SubWord
        t = [prev_key[1][3], prev_key[2][3], prev_key[3][3], prev_key[0][3]]
        for i in range(4):
            t[i] = s_box[t[i]]
        t[0] ^= Rcon[round]
        for i in range(4):
            new_key[i][0] = prev_key[i][0] ^ t[i]
        for j in range(1, 4):
            for i in range(4):
                new_key[i][j] = prev_key[i][j] ^ new_key[i][j-1]
        round_keys.append(new_key)
    return round_keys

# -------------------------
# Funciones de cifrado y descifrado
# -------------------------

def aes_encrypt():
    mensaje = input("Ingrese el mensaje a cifrar (16 caracteres): ")
    clave = input("Ingrese la clave (16 caracteres): ")
    if len(clave) != 16:
        raise ValueError("La clave debe tener exactamente 16 caracteres.")
    state = []
    key0 = []
    for a in range(4):
        state.append([])
        key0.append([])
        for i in range(4):
            state[a].append(ord(mensaje[i + a * 4]))
            key0[a].append(ord(clave[i + a * 4]))
    print("Mensaje en Int:", state)
    print("Clave en Int:", key0)
    for i in range(4):
        for j in range(4):
            state[i][j] = state[i][j] ^ key0[i][j]
    round_keys = key_generation(key0)
    for round in range(1, 10):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        for i in range(4):
            for j in range(4):
                state[i][j] ^= round_keys[round][i][j]
    sub_bytes(state)
    shift_rows(state)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_keys[10][i][j]
    for i in range(4):
        for j in range(4):
            state[i][j] = chr(state[i][j])
    ciphertext = ''.join([''.join(row) for row in state])
    print("Texto cifrado (Base64):", ciphertext)
    print("Texto cifrado:", base64.b64encode(ciphertext.encode()).decode())
def aes_decrypt():
    mensaje_cifrado_b64 = input("Ingrese el mensaje cifrado (Base64): ")
    clave = input("Ingrese la clave (16 caracteres): ")
    if len(clave) != 16:
        raise ValueError("La clave debe tener exactamente 16 caracteres.")
    ciphertext = base64.b64decode(mensaje_cifrado_b64).decode()
    print("Texto cifrado decodificado:", ciphertext)
    state = []
    key0 = []
    for a in range(4):
        state.append([])
        key0.append([])
        for i in range(4):
            state[a].append(ord(ciphertext[i + a * 4]))
            key0[a].append(ord(clave[i + a * 4]))
    print("Mensaje en Int:", state)
    print("Clave en Int:", key0)
    round_keys = key_generation(key0)
    for i in range(4):
        for j in range(4):
            state[i][j] = state[i][j] ^ round_keys[10][i][j]
    inv_shift_rows(state)
    inv_sub_bytes(state)
    for rounda in range(1, 10):
        round = 10 - rounda
        for i in range(4):
            for j in range(4):
                state[i][j] ^= round_keys[round][i][j]
        inv_mix_columns(state)
        inv_shift_rows(state)
        inv_sub_bytes(state)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_keys[0][i][j]
    for i in range(4):
        for j in range(4):
            state[i][j] = chr(state[i][j])
    plaintext = ''.join([''.join(row) for row in state])
    print("Texto descifrado:", plaintext)

# -------------------------
# Función principal
# -------------------------

def menuAES():
    while True:
        print("Menú de Cifrado AES")
        opcion = input("\n1. Cifrar\n2. Descifrar\n3. Salir\nSeleccione una opción: ")
        if opcion == '1':
            aes_encrypt()
        elif opcion == '2':
            aes_decrypt()
        elif opcion == '3':
            print("Saliendo...")
            break
        else:
            print("Opción no válida.")
